...
import { MapPin, Phone, Plus, X, Pencil, Trash2 } from 'lucide-react';
...
export default function Elders() {
  ...
  const handleSave = async () => {
    ...
    const upaData = { name, bial, phone, bio };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight uppercase">Church Elders (Upa)</h1>
          <p className="mt-1 text-stone-500 font-sans text-xs uppercase tracking-widest">List of ordained elders and their areas (bial)</p>
        </div>
        <button 
      ...
      {loading ? (
        <div className="text-center py-12 text-stone-500 font-sans">Loading elders...</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          {upas.length === 0 && (
            <div className="text-center py-12 text-stone-500 font-sans italic col-span-full">No elders found.</div>
          )}
        </div>
      )}
          ...
          <div className="bg-[#f5f5f0] rounded-[32px] w-full max-w-lg shadow-xl border border-[#e0e0d5] overflow-hidden">
            <div className="p-6 border-b border-[#e0e0d5] flex justify-between items-center bg-white">
              <h2 className="text-xl font-serif italic text-[#5A5A40]">
                {editingUpa ? 'Edit Elder (Upa)' : 'New Elder (Upa)'}
              </h2>
