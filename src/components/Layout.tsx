...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const { user, profile, isAdmin, logout, sessionExpiredNotice, clearSessionExpiredNotice } = useAuth();
  ...
  React.useEffect(() => {
    ...
    const titles: Record<string, string> = {
      ...
      '/privacy-policy': 'Privacy Policy - Bethlehem Kohhran',
      ...
      <div className="flex-1 flex flex-col min-w-0 pb-16 lg:pb-0 relative bg-[#fcfaf7]">
        ...
        <main className="relative z-10 flex-1 overflow-auto p-4 sm:p-6 lg:p-8 flex flex-col">
          {sessionExpiredNotice && (
            <div className="mb-6 p-4 bg-amber-50 border border-amber-200 text-amber-900 rounded-2xl flex items-center justify-between gap-4 font-sans text-xs shadow-sm">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-amber-100 rounded-xl text-amber-800 shrink-0">
                  <Clock className="w-4 h-4" />
                </div>
                <div>
                  <p className="font-semibold text-amber-900">Session Expired (10 Minutes Inactivity)</p>
                  <p className="text-amber-700 text-[11px] mt-0.5">You have been automatically logged out due to 10 minutes of idle time. Please sign in again to continue.</p>
                </div>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <button
                  ...
                  className="px-3 py-1.5 bg-[#5A5A40] hover:bg-[#4a4a35] text-white rounded-xl font-bold uppercase tracking-wider text-[10px] transition"
                >
                  Sign In
                </button>
                <button
          ...
          <footer className="mt-8 pt-6 pb-2 border-t border-[#e0e0d5]/50 flex flex-col items-center gap-4 text-xs text-stone-500 font-medium">
            ...
            <div className="flex flex-wrap justify-center items-center gap-x-4 gap-y-2 text-[11px] font-sans text-stone-600">
              <Link to="/privacy-policy" className="hover:text-[#5A5A40] hover:underline font-semibold transition">
                Privacy Policy
              </Link>
              <span className="text-stone-300">•</span>
              <Link to="/child-safety-standards" className="hover:text-[#5A5A40] hover:underline font-semibold transition">
                Child Safety Standards (CSAE Policy)
              </Link>
              <span className="text-stone-300">•</span>
              <Link to="/terms-of-service" className="hover:text-[#5A5A40] hover:underline font-semibold transition">
                Terms of Service
              </Link>
            </div>
